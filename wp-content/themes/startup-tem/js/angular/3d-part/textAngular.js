...
(function(){ // encapsulate all variables so they don't become global vars
	...
	*/
	...
	textAngular.directive("textAngular", [
		'$compile', '$timeout', 'taOptions', 'taSanitize', 'taSelection', 'taExecCommand', 'textAngularManager', '$window', '$document', '$animate', '$log', 'fontsFactory',
		function($compile, $timeout, taOptions, taSanitize, taSelection, taExecCommand, textAngularManager, $window, $document, $animate, $log, fontsFactory){
			return {
				require: '?ngModel',
				scope: {},
				restrict: "EA",
				link: function(scope, element, attrs, ngModel){
					...
					scope.displayElements = {
						...
						forminput: angular.element("<input type='text' class='dm-hidden-input' tabindex='-1'>"),
						html: angular.element("<textarea></textarea>"),
						text: angular.element("<div></div>"),
						// other toolbased elements
						scrollWindow: angular.element("<div class='ta-scroll-window'></div>"),
						popover: angular.element('<div class="dm-controls dm-popover popover fade top dm-text" data-popover-serial="' + _serial + '"></div>'),
						popoverArrow: angular.element('<div class="arrow"></div>'),
						popoverContainer: angular.element('<div class="popover-content"></div>'),
						popoverToolbarContainer: angular.element('<div class="popover-toolbar"></div>'),
						resize: {
					...
					scope.displayElements.popover.append(scope.displayElements.popoverToolbarContainer);
					scope.displayElements.popover.append(scope.displayElements.popoverContainer);
					element.closest('.dm-template').append(scope.displayElements.popover);
					...
					scope.showPopover = function(_el) {
						...
						if ( selectedText.startOffset != selectedText.endOffset ) {
							setTimeout(function() {
								scope.displayElements.popover.show();
								scope.displayElements.popover.addClass('in');
					...
					function getCaretPixelPos($node){
			            ...
			            pos.left = rect.left + rect.width - nodeLeft + $(window).scrollLeft();
			            pos.top = rect.top - nodeTop + $(window).scrollTop();

					    return pos;
					};

					scope.reflowPopover = function(_el){
						var $edit = angular.element(scope.displayElements.text),
							template = $edit.closest('.dm-template'),
							templateWidth = template.width(),
							templateLeft = template.offset().left,
							templateTop = template.offset().top,
							pos = getCaretPixelPos($edit),
							popoverWidth = scope.displayElements.popover.outerWidth(),
							popoverItemWidth = 60,
							popoverHeight = 50,
							containerLeft = $edit.offset().left,
							containerTop = $edit.offset().top,
							popoverSpace = 5,
							intervalId,
							arrowPosition = 20,
							leftPosition = pos.left - pos.width / 2 - popoverItemWidth / 2 + containerLeft - templateLeft,
							topPosition = pos.top - popoverHeight + containerTop - templateTop;

						if(topPosition < 0) {
							scope.displayElements.popover.removeClass('top').addClass('bottom');
							topPosition = containerTop - templateTop + pos.top + pos.height;
						} else {
							scope.displayElements.popover.removeClass('bottom').addClass('top');
						}

						scope.displayElements.popover.css({
							'top': topPosition
						});
						...
						if(leftPosition <= popoverSpace) {
							leftPosition = popoverSpace;
						}

						if(leftPosition + popoverWidth + popoverSpace > templateWidth) {
							scope.displayElements.popover.css({
								'left': 'auto',
								'right': popoverSpace + 'px'
							});
							arrowPosition = (pos.left - pos.width / 2 + containerLeft - templateLeft) - (templateWidth - popoverWidth);
						} else {
							scope.displayElements.popover.css({
						...
						scope.displayElements.popoverArrow.css({
							'left': arrowPosition + 'px'
						});

						scope.displayElements.popover.one('urlElementOpened', function() {
							_clearInterval();

							intervalId = setInterval(function() {
								if(leftPosition + scope.displayElements.popover.outerWidth() + popoverSpace > templateWidth) {
									scope.displayElements.popover.css({
										'left': 'auto',
										'right': popoverSpace + 'px'
									});
									arrowPosition = (pos.left - pos.width / 2 + containerLeft - templateLeft) - (templateWidth - scope.displayElements.popover.outerWidth());
									scope.displayElements.popoverArrow.css({
										'left': arrowPosition + 'px'
									});
								} else {
						...
						scope.displayElements.popover.on("webkitTransitionEnd transitionend oTransitionEnd", function () {
							_clearInterval();
						});

						scope.displayElements.popover.on('urlElementClosed', function() {
							_clearInterval();
						});
						
					};
					scope.hidePopover = function(){
						scope.displayElements.popover.unbind('urlElementOpened');
						scope.displayElements.popover.trigger('urlElementClosed');
						scope.displayElements.popover.removeClass('in');
						scope.displayElements.popover.removeClass('open');
						scope.displayElements.popover.removeClass('openAlways');
						scope.displayElements.popover.removeClass('transparentInsertLink');
						scope.displayElements.popover.hide();
						scope.displayElements.popoverContainer.empty();
					};
					...
					scope.displayElements.popover.on('click, focus, mousedown', function() {
						scope.clickOnPopup = true;
					})

					element.on('blur', function(e) {
						if(!scope.clickOnPopup) {
							scope.hidePopover();
							e.preventDefault();
							return false;
						}
					...
					if(attrs.taTargetToolbars) _toolbars = textAngularManager.registerEditor(_name, scope, attrs.taTargetToolbars.split(','));
						...
						scope.displayElements.popoverToolbarContainer.prepend(_toolbar)
					...
					_keyup = function(){
						var selectedText = window.getSelection().getRangeAt(0);
						scope.hidePopover();
						if(selectedText.endOffset !== selectedText.startOffset) {
							scope.showPopover();
							scope._bUpdateSelectedStyles = false;
						}
					};
					...
					_mouseup = function() {
						...
						var selectedText = window.getSelection().getRangeAt(0);
						scope.hidePopover();
						// === Adding text editing wrapper

						scope._bUpdateSelectedStyles = false;
						scope.displayElements.popover.removeClass('show');
						scope.displayElements.popoverToolbarContainer.show();
						scope.showPopover();
						scope.$apply(function(){
							scope.updateSelectedStyles();
						});
						...
						/*selColor = $(parent).closest('[data-color]').data('color');*/
						selSize = $(parent).css('font-size');
						selHeight = $(parent).css('line-height');

						selFont = $(parent).css('font-family');

						selFont = selFont
									.substr(selFont.indexOf('\'') + 1)
									.substr(selFont.indexOf('"') + 1)
									.split('\'', 1)[0]
									.split('"', 1)[0]
									.split(',', 1)[0];
						...
						//$('button[name="color"] i').addClass('icon-color').css('background', selColor);
						var fontsDrop = '<div class="fonts-drop-holder">' +
											'<ul class="fonts-drop"></ul>'+
										'</div>';

						$('button[name="font"]').each(function() {
							if ( !$(this).parent().hasClass("btn-font-holder") ) {
								$(this).find('.fonts-drop-holder').remove();
								$(this).wrap('<div class="btn-font-holder"></div>')
								$(this).parent('.btn-font-holder').append(fontsDrop);
							}
						});

						$('.fonts-drop').empty();
						for ( var type in fonts.family ) {
							for( var key in fonts.family[type] ) {
								if ( key == selFont ) {
									$('.fonts-drop').each(function() {
										$(this).append('<li class="active"><a href="#" class="btn-font-family" data-btnfamily="' + key + '" style="font-family:' + fonts.family[type][key] + '">' + key + '</a></li>');
									});
								}
								else {
									$('.fonts-drop').each(function() {
										$(this).append('<li><a href="#" class="btn-font-family" data-btnfamily="' + key + '" style="font-family:' + fonts.family[type][key] + '">' + key + '</a></li>');
									});
								}
							}
						}
						$('button[name="font"] i').addClass('icon-font').html(selFont).attr('title', selFont).css('font-family', $('.fonts-drop .active a').css('font-family'));

						$('.fonts-drop-holder').on('click', function(e) {
							e.stopPropagation();
						});
						$('.btn-font-family').on('click', function() {
							var family = $(this).data('btnfamily'),
								setFamily = '';

							function getFamily() {
								for ( type in fonts.family ) {
									for ( key in fonts.family[type] ) {
										if ( key == family ) {
											return setFamily = fonts.family[type][key];
										}
									}
								}
							}
							...
							$('.icon-font').text(family).attr('title', family).css('font-family', setFamily);
							$(this).parent().addClass('active').siblings().removeClass('active');
							$(parent).attr({'data-font': family});
							window.changeFont(setFamily);
						...
						function newFont() {
							$('body').scope().arrayChanges[0].changed = true;
			                window.updateControls(hasDifferences());
						}
						$('#saveChanges, #previewLayout').on('mouseenter', function() {
							$('[name="font"]').trigger('click');
						});
					...
					if(!_isContentEditable){
						...
						element.on('paste', function(e, eventData){
							...
							if(!_isReadonly){
								...
								if(!ie) {
									$document[0].execCommand('insertText', false, text);
									angular.element('div', angular.element(this)).each(function() {
										var divNode = angular.element(this),
											text = divNode.text();
										
										divNode.after('<br>' + text);
										divNode.remove();
									})
								}
								else {
									$document[0].execCommand('paste', false, text);
								}
						...
						// IMPORTANT CHANGES
						// Commented to restore last selection for link
						// element.on('blur', function(){
						// 	if(!scope.$parent.addUrlField){
						// 		_focussed = false;
						// 		var val = _compileHtml();
		...
		'$compile', 'textAngularManager', 'taOptions', 'taTools', 'taToolExecuteAction', '$window', 'textAngularIcons',
		function($compile, textAngularManager, taOptions, taTools, taToolExecuteAction, $window, textAngularIcons){
			return {
				scope: {
				...
				link: function(scope, element, attrs){
					...
					var setupToolElement = function(toolDefinition, toolScope){
						...
						toolElement.addClass('dm-control-button');
						...
						if(toolDefinition && !toolDefinition.display && !toolScope._display){
							...
							if(toolDefinition.iconclass){
								...
								icon.addClass('dm-widget-icon').append(textAngularIcons.getIcon(toolDefinition.iconclass));
